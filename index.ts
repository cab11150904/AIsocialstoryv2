...
// =================================================================
// Set your Gemini API key in your Firebase environment:
...
// Create a CORS middleware instance to be used by all functions.
// 'origin: true' reflects the request origin, which is a secure way to handle CORS.
const corsHandler = cors({ origin: true });

// --- Story Generation Functions (as HTTP Endpoints) ---

export const generateDescription = functions.https.onRequest((req, res) => {
    // Manually handle CORS by wrapping the function logic with the cors handler.
    corsHandler(req, res, async () => {
        ...
        if (!imagePart) {
            res.status(400).json({ error: "Request body must contain 'imagePart'." });
            return;
        }
...
- Example: "A child with short, straight blonde hair, blue eyes, and light skin with a few freckles across the nose. They are wearing a red short-sleeved t-shirt with a small dinosaur graphic on the front, and blue jeans."`;
        ...
        try {
            const response = await ai.models.generateContent({
                model,
                contents: { parts: [imagePart as Part, { text: prompt }] },
                config: { temperature: 0.2 }
            });
            res.status(200).json({ description: response.text });
        } catch (error) {
            console.error("Error in generateDescription function:", error);
            res.status(500).json({ error: "The AI failed to analyze the image." });
        }
    });
});

export const generateStory = functions.https.onRequest((req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).json({ error: 'Method Not Allowed' });
            return;
        ...
        const { childName, childAge, childGender, socialConcept, situation } = req.body as StoryGenerationParams;
...
- **Key Task**: For each page, create both the 'pageText' and a corresponding 'actionAndEmotion' summary. The 'actionAndEmotion' is CRITICAL as it will be used to generate an illustration. It must be a concise, active description of what the character is doing and feeling. For example: "happily building a tower with a friend" or "looking sad and confused while holding a broken toy".
...
- **Generate**: A title, a cover page description, a single setting for the whole story, a list of key objects, and ${pageCount} distinct story pages.`;
        ...
        try {
            ...
            res.status(200).json(JSON.parse(jsonText) as StoryOutlineResponse);
        } catch (error) {
            console.error("Error in generateStory function:", error);
            res.status(500).json({ error: "The AI failed to generate the story text." });
        }
    });
});
